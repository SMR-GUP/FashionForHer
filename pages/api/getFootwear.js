...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (id) {
        ...
        const footwear = await connection.connection.db.collection('footwear').findOne({ _id: ObjectId.createFromHexString(id)});
        if (!footwear) {
        ...
        // Fetch all t-shirts
        const footwears = await connection.connection.db.collection('footwear').find({}).toArray();
