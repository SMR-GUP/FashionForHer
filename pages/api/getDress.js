...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (id) {
        ...
        const dress = await connection.connection.db.collection('dress').findOne({ _id: ObjectId.createFromHexString(id)});
        if (!dress) {
        ...
        // Fetch all t-shirts
        const dresses = await connection.connection.db.collection('dress').find({}).toArray();
