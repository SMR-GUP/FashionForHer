...
const profile = () => {
  ...
  return (
    <div style={styles.container}>
      <h1
        className="text-4xl font-extrabold font-mono mb-3 text-indigo-900 "
        style={styles.email}
      >
        📑Order History
      </h1>
      <h1
        className="text-xl font-bold font-mono text-green-700 "
        style={styles.email}
      >
        ...
        <div style={styles.ordersContainer}>
          {orders.map((order, index) => (
            <div key={index} className="bg-yellow-100" style={styles.orderCard}>
              <h2 className="font-bold" style={styles.orderTitle}>
                Delivery Date:{" "}
                {handleCheckDeliveryDate(order.pincode, new Date(order.date))}
              </h2>
              <p>
