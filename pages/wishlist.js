...
const wishlist = () => {
              ...
              const productDetails = await Promise.all(
                wishlist.map(async (item) => {
                  ...
                  if (item.category === 'tshirt') {
                    productResponse = await fetch(`/api/getTshirt?id=${item.productId}`);
                  } else if (item.category === 'dress') {
                    productResponse = await fetch(`/api/getDress?id=${item.productId}`);
                  } else if (item.category === 'bottomwear') {
                    productResponse = await fetch(`/api/getBottomwear?id=${item.productId}`);
                  } else if (item.category === 'footwear') {
                    productResponse = await fetch(`/api/getFootwear?id=${item.productId}`);
                  }
                  ...
                  if (productResponse && productResponse.ok) {
                    ...
                    // Include the category in the product object
                    return { ...product, category: item.category };
                  } else {
                    console.error("Error fetching product details for", item.productId);
                    return null;
                  }
                })
              );
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <h1 className="text-4xl font-bold mb-6 font-mono text-center">My Wishlist💖</h1>
      {products.length===0?(  <div className="flex flex-col items-center justify-center h-64">
         ...
         <Link key={product._id} href={`/${product.category}/${product._id}`} className="block">
           <div className="flex items-center border rounded-lg overflow-hidden shadow-lg p-4 hover:shadow-xl transition-shadow duration-300 hover:bg-blue-100">
             <div className="w-1/3">
               <img src={product.image} alt={product.heading} className="w-full h-auto object-top" />
             </div>
